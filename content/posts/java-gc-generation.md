...
> **摘要**：你是否想过，Java 虚拟机（JVM）管理内存的方式，像极了一场精心设计的皇家选妃？本文不谈枯燥的算法，而是通过一个"双层舞会"的童话故事，带你理解为什么 JVM 要把对象分为"穷人"与"贵族"，以及守卫（GC）是如何在两场舞会之间进行筛选，防止灰姑娘被漏掉，同时避免舞池被无关人员挤爆。
...
起初，王子决定举办一场盛大的舞会，允许全国所有人直接进入皇宫舞池。结果可想而知：舞池瞬间爆满，绝大多数人只是来凑热闹的（朝生夕死的短命对象），王子在人海中根本找不到灰姑娘。更糟糕的是，每次为了腾出空间，守卫必须把皇宫翻个底朝天（Full GC），把所有人都赶出去重新排队，这导致整个舞会陷入停顿（Stop-The-World），效率极低。

皇家鳄鱼（这个国家的智囊是一只非常聪明的鳄鱼）发现了一个规律（**弱分代假说**）：**绝大多数人只是匆匆过客，只有极少数人值得留在皇宫。**
